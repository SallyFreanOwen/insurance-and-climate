...
######## Step one, teach R the function :run this section completely. 
...
openandsave <- function(ncname) {
  ...
  # if that doesn't work, try dname <- "rain"
  dname <- "rain"
...
#################### Step two: tell R where your netcdf file is , and then call the function on that file 

# Here I feed the ncname including a folder called Data, as my working directory sits a level above where my data is actually stored. 
ncname <- "Data/Mon_TotalPrecipCorr_VCSN_HadGEM2-ES_1971-2005_RCPpast" 
# Then we ask R to perfrom the function on that file (above):
openandsave(ncname)
